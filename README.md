...
Shortest Path Visualizer is a small web app that runs Dijkstra on a weighted
university road network graph. It exposes a Flask API and serves a lightweight frontend that lets you
select source and destination nodes and view the resulting path, distance, and metrics.
...
- Results panel and metrics table are shown side-by-side on wide screens.
...
- `GET /shortest-path?src=A&dst=B` - Compute shortest path using Dijkstra.
